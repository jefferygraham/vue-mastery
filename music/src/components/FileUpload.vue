...
export default {
  ...
  methods: {
    upload($event) {
      ...
      files.forEach((file) => {
        if (file.type === 'audio/,=mpg') {
          const storageRef = storage.ref(); // music-a8758.firebaseapp.com,
          const songsRef = storageRef.child(`songs/${file.name}`); // music-a8758.firebaseapp.com/example.mp3,
